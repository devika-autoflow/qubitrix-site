...
import { labIntro, labThreads } from "../../content/lab";
...
/**
 * Scene 04 — the one mid-page set-piece (pinned ~1.25 viewports, desktop only).
 * Orbital rebuilt from the dump concept: rAF-driven, pauses offscreen,
 * keyboard-focusable nodes, list fallback on mobile (plan §8).
 */
export default function Lab() {
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      labThreads.forEach((t, i) => {
        if (!paused && activeRef.current !== t.id) angles[i] += speeds[i] * dt;
  ...
  // Gentle pin on desktop only — the single mid-page set-piece (user decision)
  useEffect(() => {
  ...
  return (
    <section id="lab" ref={sectionRef} className="relative z-10 py-[14vh]">
      <div className="mx-auto max-w-6xl px-5 sm:px-8">
        <SectionHeading
          index="04"
          kicker={labIntro.kicker}
          heading={labIntro.heading}
          sub={labIntro.sub}
        />
        ...
        <div className="mt-12 grid items-center gap-10 lg:grid-cols-[1fr_360px]">
          ...
          <div className="os-panel p-6 sm:p-7" aria-live="polite">
            ...
            <h3 className="metal-text font-display mt-4 text-xl font-semibold">
              {active.title}
            </h3>
